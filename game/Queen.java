...
public class Queen extends Piece implements Serializable{
    ...
    public Queen(final int piecePosition, final PieceAlliances pieceAlliance) {
        super(PieceClass.QUEEN, piecePosition, pieceAlliance, true);
    }

    public Queen(final int piecePosition, final PieceAlliances pieceAlliance, final boolean isFirstMove) {
        super(PieceClass.QUEEN, piecePosition, pieceAlliance, isFirstMove);
    }


    @Override
    public Queen movePiece(Move move) {
        return new Queen(move.getMoveDestinationCoordinate(), move.getMovedPiece().getPieceAlliance());
    }

    @Override
    public String toString(){
        return PieceClass.QUEEN.toString();
    }

    private static boolean isfirstColumn(final int currentPosition, final int candidateOffset){
        return BoardOperations.COLUMN_1[currentPosition] && ((candidateOffset == -9) || (candidateOffset == 7) || (candidateOffset == -1));
    }

    private static boolean isEighthColumn(final int currentPosition, final int candidateOffset){
        return BoardOperations.COLUMN_8[currentPosition] && ((candidateOffset == 9) || (candidateOffset == -7) || (candidateOffset == 1));
    }
