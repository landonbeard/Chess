...
public class Bishop extends Piece implements Serializable{
    ...
    private final static int[] MOVE_OFFSETS = {-9, -7, 7, 9};

    //Constructor. Takes position and alliance as arguments, passing them to the Piece constructor.
    public Bishop(final int piecePosition, final PieceAlliances pieceAlliance) {
        super(PieceClass.BISHOP, piecePosition, pieceAlliance, true);
    }

    public Bishop(final int piecePosition, final PieceAlliances pieceAlliance, final boolean isFirstMove) {
        super(PieceClass.BISHOP, piecePosition, pieceAlliance, isFirstMove);
    }

    @Override
    public Bishop movePiece(Move move) {
        return new Bishop(move.getMoveDestinationCoordinate(), move.getMovedPiece().getPieceAlliance());
    }

    @Override
    public String toString(){
        return PieceClass.BISHOP.toString();
    }

    private static boolean isfirstColumn(final int currentPosition, final int candidateOffset){
        return BoardOperations.COLUMN_1[currentPosition] && ((candidateOffset == -9) || (candidateOffset == 7));
    }

    private static boolean isEighthColumn(final int currentPosition, final int candidateOffset){
        return BoardOperations.COLUMN_8[currentPosition] && ((candidateOffset == 9) || (candidateOffset == -7));
    }
